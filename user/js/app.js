...
app.config(['$routeProvider','$authProvider',function($routeProvider, $authProvider)
{
	...
	for(var provider in config.social_auth)
	{
...
app.run(['$rootScope', '$auth', '$http', '$location', 'toaster', function($rootScope, $auth, $http, $location, toaster)
{
	...
	for(var social in config.social_auth)
	{
